...
class ProjectFilter(django_filters.FilterSet):
    ...
    pattern_id = django_filters.NumberFilter(
        field_name="pattern__id", label="Pattern ID"
    )
    ...
    status = django_filters.ChoiceFilter(
        field_name="status", choices=Project.StatusChoices.choices, label="Status"
    )
    time_spent = django_filters.NumberFilter(
        method="filter_time_spent", label="Time Spent"
    )
