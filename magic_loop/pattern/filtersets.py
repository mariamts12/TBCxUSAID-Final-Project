...
class BasePatternFilter(django_filters.FilterSet):
    ...
    category = django_filters.NumberFilter(method="filter_category", label="Category ID")
    yarn_type = django_filters.NumberFilter(field_name="yarn_type__id", label="Yarn Type")
    difficulty = django_filters.ChoiceFilter(field_name="difficulty", choices=Pattern.DifficultyChoices.choices,
                                             label="Difficulty")
    popular = django_filters.OrderingFilter(
        field_name="count_saved", label="Popular"
    )
    ...
    def filter_category(queryset, name, value):
        return queryset.filter(Q(category__in=Category.objects.get_all_subcategories(value)))
